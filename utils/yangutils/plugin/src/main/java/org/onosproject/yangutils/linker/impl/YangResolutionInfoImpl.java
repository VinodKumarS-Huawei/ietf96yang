...
public class YangResolutionInfoImpl<T>
    ...
    /**
     * Current module/sub-module reference,
     * will be used in inter-file/inter-jar scenario to get the import/include list.
     */
    private transient YangReferenceResolver curReferenceResolver;

    /**
     * Stack for type/uses is maintained for hierarchical references, this is used during resolution.
     */
    private Stack<YangEntityToResolveInfoImpl<T>> partialResolvedStack;

    /**
     * It is private to ensure the overloaded method be invoked to create an object.
     */
            ...
            if (entityToResolve instanceof Resolvable) {
                ...
                switch (resolvable.getResolvableStatus()) {
                    case RESOLVED: {
                         ...
                         * it and continue with the remaining stack elements to resolve*/
    ...
    private void linkTopOfStackReferenceUpdateStack()
        ...
        if (getCurrentEntityToResolveFromStack() instanceof YangIfFeature) {
            ...
            /* Traverse up in the ancestor tree to check if the referred node is defined*/
        ...
        if (leafref.getPathType() == YangPathArgType.ABSOLUTE_PATH) {
            ...
            if (absolutePathList != null && !absolutePathList.isEmpty()) {
                ...
                if (getCurReferenceResolver() instanceof YangModule) {
                    ...
                    referredLeafFound = isLeafReferenceFound(listOfYangAbsolutePath, rootNode,
             ...
             * ancestor count and sends the list of nodes for finding the target leaf.*/
            ...
            if (absolutePathList != null && !absolutePathList.isEmpty()) {
                ...
                // Sends list of nodes for finding the target leaf.
                referredLeafFound = isLeafReferenceFound(listOfYangAbsolutePath, rootparentNode,
                        referredLeafFound, potentialAncestorWithReferredNode);
            }
        }
     ...
     *
     * @param listOfYangAbsolutePath            list of nodes in absolute path
     ...
     * @throws DataModelException a violation of data model rules
     */
    private boolean isLeafReferenceFound(Iterator<YangAbsolutePath> listOfYangAbsolutePath,
    ...
    private void resolveRightAxisNodeInPathPredicate(YangRelativePath relativePath, YangNode rootParentNode,
            YangPathPredicate pathPredicate)
            throws DataModelException {
            ...
            while (listOfYangAbsolutePathForRightRelative.hasNext()) {
                ...
                if (rootParentNode.getChild() == null) {
                    isRightAxisNodeFound = isReferredLeafOrLeafListFound(rootParentNode,
                            nodeNameInAbsolutePath, (T) pathPredicate);
                ...
                YangNode nodeFoundInTheRelativePath = isReferredNodeInSiblingProcessedForLeafref(
                        rootParentNode, nodeNameInAbsolutePath);
    ...
    private void processXPathLinking(YangEntityToResolveInfoImpl<T> entityToResolveInfo,
        ...
        if (entityToResolve instanceof YangAugment) {
            ...
            Object target = xPathLinker.processLeafRefXpathLinking(leafRef.getAbsolutePath(),
                    (YangNode) root);
            if (target != null) {
                YangLeaf leaf = null;
     ...
     * @throws DataModelException
     */
    private boolean findRefLeaf(YangNode importedNode)
            throws DataModelException {

        boolean isReferredNodeFound = false;
        List<YangAbsolutePath> absolutePathList = ((YangLeafRef) getCurrentEntityToResolveFromStack())
                .getAbsolutePath();
        if (absolutePathList != null && !absolutePathList.isEmpty()) {
            Iterator<YangAbsolutePath> listOfYangAbsolutePath = absolutePathList.listIterator();
    ...
    private YangNode findRefIdentityRef(YangNode refNode) {
        ...
        while (tmpNode != null) {
            if (tmpNode instanceof YangIdentity) {
                if (tmpNode.getName()
                        .equals(((YangIdentityRef) getCurrentEntityToResolveFromStack()).getBaseIdentity().getName())) {
                    return tmpNode;
                }
            }
            tmpNode = tmpNode.getNextSibling();
